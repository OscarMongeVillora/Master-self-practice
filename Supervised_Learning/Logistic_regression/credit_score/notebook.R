...
for(i in 1:100){
  print(i)
  PYeq1<-ifelse(my_pred<df$epsilon[i],0,1)
  confusion_matrix<-table(pred=PYeq1,actual= my_data_train$Y)
  df$accuracy[i]<-sum(diag(confusion_matrix))/(sum(confusion_matrix))
  
}

plot(epsilon,df$accuracy,xlab="Epsilon value",ylab="Accuracy",type="o",main="Optimizaci\u00f3n accuracy \n datos de entrenamiento")
df_accuracy_max<-df[df$accuracy==max(df$accuracy),]


PYeq1<-ifelse(my_pred<df_accuracy_max$epsilon[1],0,1)
...
confusion_matrix[2,2]/sum(confusion_matrix[,2]) #tpr
