...
#Usamos la función anova (análisis of deviance)
...
my_model<-glm(Y~X1+X2+X4+X6, binomial(link = "logit"), data = my_data_train)
...
r <- roc.area(Y, my_pred)
roc.plot(Y, my_pred, main = paste0("The AUC is =", r$A))
...
my_performance_data_train<-data.frame(accuracy=rep(0,n_fold),epsilon=rep(0,n_fold),
                                      tpr=rep(0,n_fold),tnr=rep(0,n_fold),AUC=rep(0,n_fold))
my_performance_data_test<-data.frame(accuracy=rep(0,n_fold),epsilon=rep(0,n_fold),
                                     tpr=rep(0,n_fold),tnr=rep(0,n_fold),AUC=rep(0,n_fold))

my_folds<-KFold(my_data$Y,n_fold)
my_folds
my_data$Y
...
for(i in 1:n_fold){
  ...
  #Balanceo clasess
  my_index<-((i-1)*nrow(my_data)/n_fold+1):(i*nrow(my_data)/n_fold)
  #my_index<-my_folds[[i]]
  
  my_data_test<-my_data[my_index,]
  my_data_train<-my_data[setdiff(rownames(my_data),rownames(my_data_test)),]
  my_data_train_0<-my_data_train[which(my_data_train$Y==0),]
  my_data_train_1<-my_data_train[which(my_data_train$Y==1),]
...
a1 <- my_performance_data_train
a2 <- my_performance_data_test
a3 <- my_performance_data_train
a4 <- my_performance_data_test
sapply(my_performance_data_train, mean)
sapply(my_performance_data_test, mean)

oscar_data <- my_data[1, ]
oscar_data$X1 <- "cuenta.buena"
oscar_data$X2 <- 10 * 4
oscar_data$X5 <- 6000

pred_oscar <- predict(my_model, newdata = oscar_data, type = "response")
pred_oscar
pred_oscar <- predict(my_model, newdata = oscar_data, type = "response")
...
my_new_data<-data.frame(X1="cuenta.buena",X2=24,X3="pre.bueno",X4="personal",X6="hombre",X7="no.solo")
...
#TAREA
my_model_all_var<-glm(Y~X1+X2+X4+X6, binomial(link = "logit"), data = my_data)
...
acc <- unique(max(df$accuracy))
epsilon_optimal<-df[df$accuracy==max(df$accuracy),]$epsilon[1]
PYeq1<-ifelse(my_pred<epsilon_optimal,0,1)
confusion_matrix<-table(PYeq1,Y)
tpr<-confusion_matrix[2,2]/(confusion_matrix[2,2]+confusion_matrix[1,2]) #tpr
tnr<-confusion_matrix[1,1]/(confusion_matrix[2,1]+confusion_matrix[1,1]) #tnr
AUC<-roc.area(my_data_train$Y,my_pred)$A
