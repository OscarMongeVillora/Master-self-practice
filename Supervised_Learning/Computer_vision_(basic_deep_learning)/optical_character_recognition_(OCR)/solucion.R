...
colnames(data_test) <- c("Y", paste("X", 1:256, sep = "")) #Rename variables
class(data_test[, 1])
levels(data_test[, 1])
...
#Print just a "mean" of the digits
for (digit in 0:9) {
  print(digit)
  images_digits_0_9[digit + 1, ] <- apply(data_train[data_train[, 1] == digit, -1], 2, sum) #sum each col
  
  images_digits_0_9[digit + 1, ] <- images_digits_0_9[digit + 1, ]/max(images_digits_0_9[digit + 1, ]) * 255 #normalize and multiply by total of variables
  
  z <- array(images_digits_0_9[digit + 1, ], dim = c(16, 16))
  z <- z[, 16:1]  ##right side up
  image(1:16, 1:16, z, main = digit, col = CUSTOM_COLORS(256)) #plot 16x16 pixeles
}



#plot ten first values
...
prediction.digit<-model.knn[row]
...
#Let's gonna see which values are wrong predicted for SVM
